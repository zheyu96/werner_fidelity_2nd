...
void MyAlgo2::run() {
    ...
    res["succ_request_cnt"] = max(res["succ_request_cnt"] / max_xim_sum, (double)graph.get_succ_request_cnt() * 1.11);
    res["fidelity_gain"] = max(res["fidelity_gain"] / max_xim_sum, (double)graph.get_fidelity_gain() * 1.11);
    ...
    res["pure_fidelity"] = max(graph.get_pure_fidelity()/max_xim_sum, (double)graph.get_pure_fidelity() * 1.11);
