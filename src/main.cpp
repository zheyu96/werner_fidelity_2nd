...
int main(){
    ...
    #pragma omp parallel for
    for(int r=0;r<round;r++){
        PathMethod* path_method = new Greedy(); // 若要同時掃 QCAST/REPS，可再包一層 for
        const string X_name = "num_nodes";
        // 先準備輸出檔（每個 Y_name 一個 .ans）
        for (string Y_name : Y_names) {
            string filename = "ans/" + path_method->get_name() + "_" + X_name + "_" + Y_name + ".ans";
            fstream file(file_path + filename, ios::out); // truncate 建新檔
        }

        // 掃描不同的節點數
        for (double change_value : change_parameter[X_name]) {
            map<string, map<string, double>> sum_res;  // algo_name -> (Y_name -> sum over rounds)
            // 每個 round 都重生一張對應節點數的圖、重生 requests
            #pragma omp parallel for
            for (int r = 0; r < round; r++) {
                // 1) 以 num_nodes = change_value 重生圖
                int num_nodes = (int)change_value;
                string in_name = "round_" + to_string(r) + "_N" + to_string(num_nodes) + ".input";
                string input_path = file_path + "input/" + in_name;

                string command = "python3 graph_generator.py ";
                string parameter = to_string(num_nodes);
                // 這裡用獨立檔名避免覆蓋其他參數掃描用的 round_r.input
                if (system((command + input_path + " " + parameter).c_str()) != 0) {
                    cerr << "error:\tsystem process python error (num_nodes=" << num_nodes << ", round=" << r << ")\n";
                    exit(1);
                }

                // 2) 以目前的 default_setting + 其他 input_parameter 建圖
                map<string,double> input_parameter = default_setting;
                input_parameter[X_name] = change_value; // 覆蓋節點數
                int    avg_memory         = input_parameter["avg_memory"];
                int    request_cnt        = input_parameter["request_cnt"];
                int    time_limit         = input_parameter["time_limit"];
                double min_fidelity       = input_parameter["min_fidelity"];
                ...
                Graph graph(input_path, time_limit, swap_prob, avg_memory, 
                            min_fidelity, max_fidelity, fidelity_threshold,
                            A, B, n, T, tao, Zmin, bucket_eps, time_eta);

                // 3) 依新的圖重生 requests（維持你的 path 長度帶寬限制）
                int length_upper = default_setting["path_length"] + 1;
                int length_lower = default_setting["path_length"] - 1;
                vector<pair<int,int>> requests = generate_requests(graph, request_cnt, length_lower, length_upper);

                // 4) 建 path（先擴資源以利找路徑）、去重
                Graph path_graph = graph;
                path_graph.increase_resources(10);

                unique_ptr<PathMethod> new_path_method;
                if (path_method->get_name() == "Greedy")      new_path_method.reset(new Greedy());
                else if (path_method->get_name() == "QCAST")  new_path_method.reset(new QCAST());
                else if (path_method->get_name() == "REPS")   new_path_method.reset(new REPS());
                else { cerr << "unknown path method\n"; assert(false); }

                new_path_method->build_paths(path_graph, requests);
                map<SDpair, vector<Path>> paths = new_path_method->get_paths();
                // 去重
                map<SDpair, set<Path>> paths_st;
                for (auto &kv : paths) for (Path p : kv.second) paths_st[kv.first].insert(p);
                paths.clear();
                for (auto &kv : paths_st) for (Path p : kv.second) paths[kv.first].push_back(p);
                ...
                // 6) 彙整輪次結果（臨界區域避免 data race）
                #pragma omp critical
                {
                    for (auto *algo : algorithms) {
                        for (string Y_name : Y_names) {
                            sum_res[algo->get_name()][Y_name] += algo->get_res(Y_name);
                        }
                    }
                }
            ...
            for (string Y_name : Y_names) {
                ...
                ofs << change_value << ' ';
                for (string algo_name : algo_names) {
                    double avg_val = sum_res[algo_name][Y_name] / round;
                    ofs << avg_val << ' ';
                }
                ofs << '\n';
            }
        } // end for each num_nodes
